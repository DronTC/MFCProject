...
namespace MFCLibrary.Data.resourse
{
    internal class En
    {
        internal static Dictionary<ResourceId, string> resourcesEn = new Dictionary<ResourceId, string>
        {
            ...
            {ResourceId.addServicingCompleate, "A maintenance operation has been added" },
            {ResourceId.employees, "Employees" },
            {ResourceId.salutationDateTime, "1. Current date and time\n2. Own date and time" },
            {ResourceId.takeUsualTime, "Enter the time (format HH:MM)" },
            {ResourceId.timeError, "You must enter the time between 9:00 and 19:00. Try typing again, or go back to the menu: <...>" },
            {ResourceId.searchCriteriaServicing, "Search criteria for a service operation:\n1.Client's full name\n2.Employee's full name\n3.Service date and coupon number" },
            {ResourceId.takeTicket, "Enter the coupon number (format XNNN: X is the first letter of the service name, N is the serial number of the coupon for the current day))" },
            {ResourceId.ticket, "Coupon" },
            {ResourceId.service, "Service" },
            {ResourceId.window, "Window" },
            {ResourceId.employee, "Employee" },
            {ResourceId.client, "Client" },
            {ResourceId.variantsSearchServicing, "Search options:\n1.Date\n2.Date range\n3.By the windows of the MFC\n4.All statistics" },
            {ResourceId.statisticUsualClients, "Service statistics for ordinary users" },
            {ResourceId.statisticAuthorizedClients, "Service statistics for verified users (public services)" },
            {ResourceId.from, "from" },
            {ResourceId.before, "before" },
            {ResourceId.statisticWindow, "Window maintenance statistics" },
            {ResourceId.averageLoad, "Average load" },
            {ResourceId.perHour, "In an hour" },
            {ResourceId.perDay, "In the day" },
            {ResourceId.perWeek, "Per week" },
            {ResourceId.perMonth, "Per month" },
            {ResourceId.perYear, "Per year" },
            {ResourceId.inTotal, "Total" },
            {ResourceId.fullName, "Full name" },
            {ResourceId.passport, "Passport" },
            {ResourceId.birthday, "Birthday" },
